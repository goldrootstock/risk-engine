...
| 지평 불일치 대응 (미결, JK 결정) | 현재: 그대로 두고 기록 | `backtest` | (a) **√h 스케일**: h 영업일 전이의 VaR 를 √h 배 — 실측 초과 63 → 50(h≥2 부분 15 → 2), 전체 0.81 %. Basel 의 지평 일치 원칙에 부합하나 √h 는 i.i.d. 가정 [출처: BCBS 1996 §I 는 1일 VaR 와 1일 손익의 비교를 요구]. (b) **h ≥ 2 제외**: n = 5,839, 초과 48 (0.82 %), 표본 5.5 % 손실. (c) **그대로 + 문서화**: 63/6,179 = 1.02 % 로 빈도는 맞지만 h≥2 구간이 과소추정을 숨긴다. 권고: **(a)** 를 정식으로 하되 raw 도 같이 보고 — 결과가 나쁜 쪽(h≥2)을 버리는 (b) 는 노트 00 §3 형태라 피한다. 2005-09-22→10-07 의 h = 11 (교집합이 10 영업일을 버린 구멍, 원인 미확인 — ETL 결측 분포 조사 예약) 은 별도 표기 | 초과 수·Kupiec 결과·모델 문서의 서사 |
