...
| 항목 | 정의 | 근거 |
|---|---|---|
| 시나리오 | `config/stress_scenarios.toml` 의 historical + hypothetical 전부. **손실의 지평은 §7-1**: 정식은 역사적 구간 안의 최악 MPOR 창 | 충격 크기를 코드가 정하지 않는다(노트 00 §3 #7). 파일 sha 기록 |
...
### 7-1. 시나리오 손실의 지평 — MPOR 기준이 정식 (JK 지시, 2026-09-15)

첫 실행에서 결정 시나리오가 `rates_2022`(2022-01 → 10 누적)로 나왔다. 그러나 디폴트 펀드의 기준(PFMI Principle 4)은 "극단적이지만 발생 가능한 상황에서 **청산 기간 동안** 발생하는 손실" 이고 이 프로젝트의 MPOR 은 2 일이다. 청산소는 부도 회원의 포지션을 10 개월 보유하지 않는다. 따라서:

| basis | 역사적 시나리오의 손실 | 가상 충격 | 용도 |
|---|---|---|---|
| **`mpor` (정식)** | 구간 안의 **최악 h 일(= MPOR) 창** — 연속 h 개 변화량 벡터를 합쳐 한 번 매핑(블록 부트스트랩·실현 MPOR 손실과 같은 구성). 구간이 h 일보다 짧으면 구간 누적 | 순간 충격 그대로 | 디폴트 펀드 규모 |
| `path` | 구간 전체 누적(노트 07 §1 과 같음) | 순간 충격 그대로 | 별도의 경로·유동성 분석 |

셋을 기록한다(`default_fund_runs.params.basis` ∈ {`mpor_historical`, `mpor`, `path`}, 실제 사용한 창은 `params.windows`). **JK 결정(2026-09-15, #8)**: 정식은 `mpor_historical` — 역사적 시나리오만, 가상 충격 배제. 배제의 근거는 범주(Basel IRRBB 감독 충격은 청산 기간 손실이 아니다)이고 지평 불일치(순간 200 bp 대 표본 최악 2 일 ≈ 50 bp)는 그 결과다. 가상 충격을 MPOR 크기로 재조정하지 않는다. 이 선택은 더 작은 값을 고르는 것이므로 배제한 두 값을 같은 표에 남긴다(모델 문서 §6-4). 결과: §13.
...
| 8 | Cover-2 시나리오 집합 | **결정(JK 2026-09-15)**: 정식 `mpor_historical` = 역사적 시나리오의 MPOR 창만(24.3 M, CM_HEDGED). 근거는 범주 — 가상 충격은 Basel IRRBB 감독 충격이지 청산 기간 손실이 아니다. 재조정 안 기각(재조정한 표준은 표준이 아니다). `mpor`(+가상, 28.8 M)·`path`(49.6 M)는 별도 기록 | — |
...
## 13. 첫 실행 결과 (2026-09-15, 구현 단계 1~5 완료)

정본은 모델 문서 §6-3·§6-4 와 `decisions.md` §8-1·§8-2. 요약: MAIN 2026-09-09 IM 3.51 M(코어 1.61 + 플로어 0.83 + 블렌드 1.01 + 유동성 0.06), SPAN 8.68 M. 커버리지(다섯 장부 전부 99.85~99.92 %, Kupiec 기각은 보수 쪽만): MAIN 6,178 일 breach 5, 코어만 31, SPAN 4, 22/25 창 breach 0, 최대 부족분 2020-04-16→20 의 7.88 M(IM 의 3.34 배). 애드온 분해(MAIN): 플로어 18 건, 블렌드 6 건, 유동성 0 건, h 일 보정 2 건. Cover-2 — **정식 `mpor_historical` 24.3 M**(`covid_oil_2020` 04-17→20 창, **CM_HEDGED** + CM_ENERGY); 별도 기록 `mpor` 28.8 M(가상 `rates_up_200`), `path` 49.6 M(`rates_2022`). 상관 의존 헤지 장부가 디폴트 펀드를 결정하며, 그 장부는 breach 빈도로는 가장 안전해 보인다(6 건, 최악 5.43 배) — A1 §7-1 과 같은 구조. 레거시 SPAN 은 장부에 따라 breach 2~27 건(13 배)으로 방법론의 일관성이 없다(모델 문서 §6-6). 숫자 기준: 단일일(2026-09-09)과 기간 평균을 모든 표에 구분해 적는다.

결과를 보고 바꾼 값은 없다. §10 의 승인 항목 8 개는 그대로 대기이며, 특히 #2(APC 구조 — 과다 마진의 원인)와 #1(코어 측정치)은 이 결과가 판단 재료다: 코어만으로도 0.50 % 라 VaR 99 % 로 바꾸거나 블렌드를 빼도 99 % 를 지킬 여지가 있다. 바꾸면 새 마진 실행(다른 sha)이고 이 절에 기록된다.

구현 위치: `src/risk_engine/margin/{params,core,span,engine,coverage,default_fund,__main__}.py`, `db/migrations/0009_margin.sql`, `config/margin_params.toml`, `config/positions_members.csv`, 테스트 `tests/test_margin_*.py`, API `/margin`·`/margin/coverage`·`/margin/coverage/days`·`/default-fund`, 대시보드 마진 구역. §9 의 "기록은 `record.write`" 는 A1 의 `risk.record.write` 를 그대로 쓴다는 뜻이다(마진 전용 record 모듈 없음).
