...
둘 다 기록한다(`default_fund_runs.params.basis`, 실제 사용한 창은 `params.windows`). 결과와 그 해석 — 정식 기준에서 결정 회원이 바뀌는지 — 는 §13 과 모델 문서 §6-4. **남는 문제**: 가상 충격(`rates_up_200` 등, Basel IRRBB 의 장기 지평 충격)은 순간 충격이라 MPOR 기준에서도 지평이 어긋난다. §10 #8 로 재상정한다.
...
| 8 | Cover-2 시나리오 집합 | **재상정(2026-09-15, §7-1·§13)**: 정식 = 역사적 시나리오의 MPOR 창만. 가상 충격은 MPOR 크기로 재조정(예: 250 일 창의 99 % 2 일 이동)하거나 별도 표 | 현재 기록된 정식: historical + hypothetical (가상 `rates_up_200` 이 결정, 28.8 M) |
...
정본은 모델 문서 §6-3·§6-4 와 `decisions.md` §8-1·§8-2. 요약: MAIN 2026-09-09 IM 3.51 M(코어 1.61 + 플로어 0.83 + 블렌드 1.01 + 유동성 0.06), SPAN 8.68 M. 커버리지(다섯 장부 전부 99.85~99.92 %, Kupiec 기각은 보수 쪽만): MAIN 6,178 일 breach 5, 코어만 31, SPAN 4, 22/25 창 breach 0, 최대 부족분 2020-04-16→20 의 7.88 M(IM 의 3.34 배). 애드온 분해(MAIN): 플로어 18 건, 블렌드 6 건, 유동성 0 건, h 일 보정 2 건. Cover-2 — 경로 49.6 M(`rates_2022`) → **MPOR 정식 28.8 M**(가상 `rates_up_200`, CM_RATES + CM_DIVERSIFIED) → MPOR 역사적만 **24.3 M**(`covid_oil_2020` 04-17→20 창, **CM_HEDGED** + CM_ENERGY). 2 일로 자르면 상관 의존 헤지 장부가 역사적 시나리오의 결정 회원이 된다; 가상 순간 충격이 그 위에 서는 것이 #8 의 남은 문제다.
