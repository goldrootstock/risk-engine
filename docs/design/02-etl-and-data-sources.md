...
**합계 31 계열** (국채 11 · FX 12 · 에너지 8). **표본 시작일 2006-02-09** — 30년물 재개일. 그 이전으로 늘리면 30Y 공백이 intersection 을 끊는다. 2006-02 ~ 2026-09 ≈ 20.5년, intersection 후 약 5,000 영업일 [추정]. 2008 · 2020-03 · 2022 금리 스트레스 창 전부 포함.
...
**수익률 노트 03 로 넘기는 두 가지**: (1) 에너지 현물은 음수가 실존한다(WTI 2020-04-20 = −37.63) — 로그수익률이 정의되지 않으므로 단순수익률 또는 절대변화 중 택일. (2) FX 크로스의 부호·환산 순서.
...
| 재실행 창 | 증분 실행은 `max(last price_date − 10 영업일, --since)` 부터 다시 받는다 | 벤더 재공시(restatement) 흡수. 10일은 EIA·Treasury 의 통상 정정 범위 [추정] |
