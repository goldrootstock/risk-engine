...
- 상태: §8 승인 요청 6개 **승인**(2026-09-12). 이후 JK 지시로 유니버스 31계열 확대 · 공공누리 4유형 보류 · E3 재평가 · §10 마진 의존성 반영 — **확정본**. **§10-4 A2 착수 시점의 M2 해석 (2026-09-15, 승인 대기)**
...
### 10-4. A2 착수 시점의 해석 — "M2 뼈대" 가 코드로는 무엇인가 (2026-09-15, 승인 대기)

노트 00~08 을 다시 읽었을 때 §10-3 의 1단계가 두 가지로 읽혔다. 다음 사람이 같은 갈림길에 서지 않도록 적는다.

| 읽기 | 내용 | 판정 |
|---|---|---|
| **(i) 현물 장부를 "청산 상품인 것처럼" 직접 마진** | 입력은 `instruments`·`prices`·`positions` 그대로. FX 현물·에너지 현물·CMT 국채 포지션에 2일 MPOR FHS ES 를 적용 | **채택 권고** |
| (ii) 합성 선물을 상품으로 만든다 | F = S·exp((r−c)τ) 로 선물 가격 계열을 만들어 `instrument_type='future'` 상품과 가격을 추가 | 기각 권고 |

(ii) 를 기각하는 이유 둘. 첫째, 노트 01 §1 — `prices` 는 벤더 값만 저장하고 파생값은 저장하지 않는다. 합성 가격 행을 넣으면 그 원칙이 깨지고 ETL 의 "받은 것 그대로" 감사 추적에 우리가 만든 값이 섞인다. 둘째, §10-2 가 이미 지적한 대로 c = 0·τ 고정이면 ln F_t − ln F_{t−1} = ln S_t − ln S_{t−1} + τ·(r_t − r_{t−1}) 이라 **일별 변화량이 현물과 사실상 같다**(하루 금리 변화 × τ 만큼의 수 bp 차이). 마진은 변화량 분포의 함수이므로 (ii) 의 마진 = (i) 의 마진 + 잡음이다. 합성 선물은 결과를 바꾸지 않으면서 "선물을 마진했다" 는 인상만 만든다 — §3-3 의 사실 불변 원칙에도 어긋난다.

따라서 A2 의 코어 마진·플로어·블렌드·커버리지 백테스트·Cover-2 는 (i) 로 구현하고, cross-margining·스프레드 차지·유동성/집중 add-on 은 §10-3 3단계대로 **설정값**(`config/margin_params.toml`)이며, 모델 문서 "알려진 취약점" 에 위 두 문장을 그대로 넣는다. 이력서 문구는 §10-3 3단계.

Databento(M1) 약관 확인은 이 시점에 아직 이루어지지 않았다 [미확인 2026-09-15: handoff §9-C 리마인드 "주식 경로 진행 시" 상태 그대로]. M1 은 A2 의 전제가 아니다.
