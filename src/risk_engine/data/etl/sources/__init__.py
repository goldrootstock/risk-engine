...
def build_sources(eia_api_key: str | None) -> dict[str, Source]:
    """Instantiate every source keyed by ``instruments.source``.

    EIA is included only when a key is configured, so ``sync --source eia`` fails with a
    clear message instead of a 403 from the vendor.
    """
    sources: dict[str, Source] = {"ecb": EcbSource(), "ustreasury": UsTreasurySource()}
    if eia_api_key:
        sources["eia"] = EiaSource(api_key=eia_api_key)
    return sources


__all__ = ["EcbSource", "EiaSource", "UsTreasurySource", "build_sources"]
